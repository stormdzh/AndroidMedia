package com.example.accaudiodemo;
...
public class MainActivity extends Activity implements OnClickListener {
    ...
    private Button init, start, stop;
...
//        parent = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + "/ACC音频");
        parent=new File("/storage/emulated/0/ffmpeg/aac");
        if (!parent.exists()) {
            parent.mkdirs();//创建文件夹
        }
    }
    ...
    private int startRecord() {
        ...
        mAudioRecord = new AudioRecord(AudioSource.MIC, mSampleRateInHz, mChannelConfig, mBitSize, mBufferSizeInBytes);
