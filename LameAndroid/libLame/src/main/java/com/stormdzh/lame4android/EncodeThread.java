...
public class EncodeThread extends HandlerThread implements AudioRecord.OnRecordPositionUpdateListener {
    ...
    private static class StopHandler extends Handler {
        ...
        public StopHandler(Looper looper, EncodeThread encodeThread) {
            super(looper);
     ...
     */
    public EncodeThread(File file, int bufferSize) throws FileNotFoundException {
    ...
    private int processData() {
        if (mTasks.size() > 0) {
            ...
            int encodedSize = Lame.encode(buffer, buffer, readSize, mMp3Buffer);
            if (encodedSize > 0) {
                try {
